$(function () {
    var potentialDialog, potentialDatagrid, potentialDialogForm, potentialDatagridBtn, potentialShareOrturnDialog,
        potentialShareOrturnDialogBtn, potentialShareOrturnDialogForm, potentialDisableA;
    ...
    potentialShareOrturnDialogBtn = $("#potential_shareOrTransfer_dialog_bb");
    potentialShareOrturnDialogForm = $("#potential_shareOrturn_dialog_form");
    potentialDisableA = $("#potential_edit,#potential_share,#potential_move, #potential_fail, #potential_become");
    ...
    potentialDatagrid.datagrid({
        ...
        onClickRow: function (rowIndex, rowData) {
            console.log("potential 1:" + rowData.status);
            if (rowData.status != -1) {
                console.log("potential 2:" + rowData.status);
                potentialDisableA.linkbutton('enable');
            } else {  // 用户状态离职，不可操作编辑和离职按钮
                potentialDisableA.linkbutton('disable');
            }
        },
        ...
        columns: [[
            ...
            {field: "status", title: "状态", width: 1, align: "center", formatter: statusFormatter}
        ]],

    });

    function inputuserFormatter(value, row, index) {
        return value ? value.realname : null;
    }
    ...
    potentialShareOrturnDialog.dialog({
        ...
        height: 250,
        resizable: true,
        buttons: "#potential_shareOrTransfer_dialog_bb",
        closed: true
    });
    ...
    var cmdObj = {
        ...
        // 共享和移交的保存方法
        shareOrTransferSave: function () {
            potentialShareOrturnDialogForm.form("submit", {
                url: "/potential_updateInCharge",
                // 传递选中角色的Id到后台中
                success: function (data) {
                    data = $.parseJSON(data)
                    if (data.success) {
        ...
        shareOrTransferCancel: function () {
            potentialShareOrturnDialog.dialog("close");
        },
        /*开发失败*/
