	...
	var cmdObj = {
		...
		share: function() {
			...
			if (rowData) {
				...
				console.log(rowData);
				if (rowData.inchargeruser) {
					rowData["oldseller.name"] = rowData.inchargeruser.username;
					rowData["oldseller.id"] = rowData.inchargeruser.id;
				}
				if (rowData.id) {
					rowData["customer.id"] = rowData.id;
				}
		...
		fail: function() {
			...
			if (rowData) {
				$.messager.confirm("温馨提示", "您确认要标记此用户开发失败吗?", function(yes) {
					$.get("/customer_fail?id=" + rowData.id, function(data) {
						if (data.success) {
							$.messager.alert("温馨提示", data.msg, "info");
		...
		become: function() {
			...
			var rowData = customerDatagrid.datagrid("getSelected");
			if (rowData) {
				$.messager.confirm("温馨提示", "您确认要转正此客户吗?", function(yes) {
					if (yes) {
						$.get("/customer_become?id=" + rowData.id, function(data) {
							if (data.success) {
								$.messager.alert("温馨提示", data.msg, "info");
								customerDatagrid.datagrid("reload");
							} else {
