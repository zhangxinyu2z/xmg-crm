...
public class PermissionUtil {
	 ...
	 * 1.对超级管理员直接放行
	 * <p>
	 * 2.获取当前系统中的所有权限资源（即需要验证的url），如果请求不在权限资源中，直接放行
	 * <p>
	 * 2.1.如果请求在权限资源中，获取当前用户的所有权限，判断是否拥有该权限，先进行完全匹配，再判断是否拥有all权限，没有则拦截
	 * 
	 * @param function
	 * @return
	 */
	public static boolean checkPermission(String function) {
		...
		if (CommonUtils.ALL_PERMISSIONS.size() == 0) {
			...
			for (Permission p : allPermissions) {
				CommonUtils.ALL_PERMISSIONS.add(p.getResource());
			}
		}
		// 判断当前访问表达式是否需要进行权限判断
